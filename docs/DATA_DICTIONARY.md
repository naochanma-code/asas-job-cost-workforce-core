...
| Entity | Fields | Constraint / query / sensitivity |
| --- | --- | --- |
| expense_submissions | project_id, job_id?, code:text, employee_id:ref(employees), expense_date:date, category_id:ref(cost_categories), amount_satang:money, description:text, status:DRAFT/PENDING_REVIEW/APPROVED/REJECTED/CANCELLED, revision:int, supersedes_id:ref(expense_submissions)?, submitted_at:timestamptz?, reviewed_by:ref(users)?, reviewed_at:timestamptz? | amount>0; UNIQUE(code,revision); submit มี READY evidence 1–5; index(project_id,status,expense_date),(employee_id,expense_date); FIN/PII |
| expense_evidence | expense_id:ref(expense_submissions), project_id, job_id?, owner_user_id:ref(users), revision:int, sequence:int, object_key:text, sha256:char(64), mime_type:text, byte_size:bigint, source_channel_id:text?, line_message_id:text?, state:RECEIVED/FETCHING/READY/RETRY/FAILED, replaces_id:ref(expense_evidence)? | sequence 1–5 active; bytes 1..10MB; UNIQUE(object_key), UNIQUE(channel_id,line_message_id) when present; hash ไม่ unique ข้ามผู้ส่งเพราะรูปเดียวอาจต้อง review; expense/project/job match; FIN/PII |

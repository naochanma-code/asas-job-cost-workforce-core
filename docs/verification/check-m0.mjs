...
check('prototype JavaScript parses and has no remote dependency',()=>{const html=fs.readFileSync(path.join(root,'docs/prototype/index.html'),'utf8');const scripts=[...html.matchAll(/<script>([\s\S]*?)<\/script>/g)];assert.equal(scripts.length,1);new vm.Script(scripts[0][1]);assert.ok(!/<(?:script|link|iframe)[^>]+(?:src|href)=["']https?:/i.test(html));assert.ok(!/\b(?:fetch|XMLHttpRequest|WebSocket)\b/.test(scripts[0][1]))});

...
ผลตรวจ resource วันที่ 26 กันยายน: มี Railway environment `staging` เพียงชุดเดียว; ยังไม่มี Web/API recovery แยก. ฐาน recovery เดิมมีสำเนาข้อมูลจริงจึงใช้เป็น fixture ไม่ได้. ดู [ข้อเสนอปลายทาง provider](M1_PROVIDER_RECOVERY_PROPOSAL.md). C2 คง `NOT_READY` จนมีปลายทางว่างและอนุมัติค่าใช้จ่าย/ขอบเขตจริง.
